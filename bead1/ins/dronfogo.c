...
{	
	...
	if (*size == 0){
		...
		{		    	// idomar-child process
			...
			printf("\n\tKövetkező megbízás: %s", pipestring);
			close(pipe1[0]);
		...
		{	
			...
			if (madar == 0)	// madar-child process forked from parent process
			{
				close(pipe1[1]);
				close(pipe1[0]);
				close(pipe2[1]);
			...
			else            // parent process (kozpont)
			{
				...
				waitpid(idomar,&status1,0); 
				waitpid(madar,&status2,0); 
