...
{	
	...
	if (*size == 0){
		...
		{		    	// idomar process
			...
			printf("\n\tPID: %i: Az idomár most végzi a madár körüli teendőket...\n",getpid());
			sleep(2);
			...
			read(pipe1[0], sz, sizeof(sz));
			printf("\n\tPID: %i: Következő megbízás: %s",getpid(), sz);
			close(pipe1[0]);
			
			read(pipe3[0], &r, sizeof(int));
			close(pipe3[0]);
			
			write(pipe2[1], &r, sizeof(int));
			close(pipe2[1]);
		...
		else 
		{
			...
			else          // szulo process (kozpont)
			{
				...
				waitpid(idomar,&idomars,0);
				printf("\n\tPID: %i waited for to end: %i.\n", getpid(),idomar);
