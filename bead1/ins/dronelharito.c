...
void readData(char* fname, struct ugyfel* u, int* size) {
  int f = open(fname, O_RDONLY|O_CREAT|O_TRUNC,0666);
  printf("\n########## %i\n", f);
  if (f < 0) {
  ...
  while(sizeof(c) == read(f, &c, sizeof(c))) {
    printf("\tkiolvasott ugyfel: %s", c.nev);
    *u = c;
    (*size)++;
    u++;
  }
...
void mentes(char* fname, struct ugyfel* u, int* size) {
  int f = open(fname, O_WRONLY,0666);
  printf("\n###### %i\n", f);
  if (f < 0) {
  	perror("Nem lehet megnyitni a fajlt!\n");
  	exit(1);
  ...
  int i = 0;
  while(i < *size) {
    write(f, u, sizeof(struct ugyfel));
